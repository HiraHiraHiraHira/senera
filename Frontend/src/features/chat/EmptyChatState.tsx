...
export function EmptyChatState({
  ...
  return (
    <div className="flex max-w-xl flex-col items-center text-center">
      <LogoMark size={30} />
      <h2 className="mt-5 font-serif text-[24px] italic text-ink-900" style={{ fontWeight: 500 }}>
        今天想做点什么？
      </h2>
        ...
        <div className="mt-7 flex flex-wrap items-center justify-center gap-2">
          {suggestions.map((suggestion) => (
            onSelectSuggestion ? (
              ...
              <span
                key={suggestion}
                className="inline-flex items-center gap-1.5 rounded-full border border-ink-200/70 bg-paper-50/80 px-3 py-1 text-[12.5px] text-ink-700"
              >
                {suggestion}
              </span>
            )
